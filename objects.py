...
player_images =[]
for i in range (1,5):
    player_images.append(pygame.transform.scale(pygame.image.load(f'assets/player_images/{i}.png'),(cell*2,cell*2)))
pac0=pygame.transform.scale(pygame.image.load(f'assets/player_images/{0}.png'),(cell*2,cell*2))
...
#sue
sue_img=pygame.transform.scale(pygame.image.load(f'assets/ghost_images/sue.png'),(2*cell_side_size,2*cell_side_size))
#funky
funky_img=pygame.transform.scale(pygame.image.load(f'assets/ghost_images/funky.png'),(2*cell_side_size,2*cell_side_size))


dead =[]
for i in range (0,4):
    dead.append(pygame.transform.scale(pygame.image.load(f'assets/ghost_images/{i}.png'),(2*cell_side_size,2*cell_side_size)))
...
class Pacman(MovingObject):
    ...
    def draw(self, screen,counter): #render
        ...
        if self.direction == -2: #right looking
            screen.blit(player_images [counter //5],(p[0]-cell/2,p[1]-cell/2))

        if self.direction == 2: #left
            screen.blit(pygame.transform.flip(player_images [counter //5],True,False),(p[0]-cell/2,p[1]-cell/2))

        if self.direction == 1:#up
            screen.blit(pygame.transform.rotate(player_images [counter //5],90),(p[0]-cell/2,p[1]-cell/2))

        if self.direction == -1:#down
            screen.blit(pygame.transform.rotate(player_images [counter //5],270),(p[0]-cell/2,p[1]-cell/2))
        if  self.direction == 0:#stop
            screen.blit(pac0,(p[0]-cell/2,p[1]-cell/2))
...
class Ghost(MovingObject):
    ...
    def startShield(self):
        ...
        if self.mode.current_mode == scattering_mode:
            self.scatter()
    ...
    def draw(self, screen,img):
        if self.visible:
            ...
            if self.mode.current_mode == scared_mode :
                screen.blit(scared_icon,(p[0]-cell/2,p[1]-cell/2))
            elif self.mode.current_mode == respawning_mode:
                screen.blit(dead_icon,(p[0]-cell/2,p[1]-cell/2))
            else:
                screen.blit(img,(p[0]-cell/2,p[1]-cell/2))
...
class SwitchMode(object):
    ...
    def setShieldMode(self):
        ...
        if self.current_mode is chasing_mode:
